...
Camera::Camera()
{
	SetLen(0.25f * DirectX::XM_PI, 1.0, 1.0, 1000.0f);
}
...
void Camera::SetLen(float fovY, float aspect, float zn, float zf)
{
	...
	mNearWindowHeight = 2.0 * mNearZ * tanf(0.5 * mFovY);
	mNearWindowHeight = 2.0 * mFarZ * tanf(0.5 * mFovY);
...
void Camera::LookAt(DirectX::FXMVECTOR pos, DirectX::FXMVECTOR target, DirectX::FXMVECTOR worldUp)
{
	...
	XMVECTOR right = XMVector3Cross(worldUp, lookat);
...
void Camera::Strafe(float d)
{
	...
	// XMVectorMultiplyAdd 前两个向量X乘，加到第三个向量(V1 * V2) + V3
...
void Camera::UpdateViewMatrix()
{
	if (mViewDirty)
	{
		...
		// 计算position与各轴的距离，
		// 由于position是世界坐标的位置，所以取负，即做逆变换平移
